...
void DebugNewTracer::Add (void* p, const char* file, int line, size_t size)
{
    ...
    _totalsize += size;
#if defined(LOG_FILE) 
    //LYDBG(L"*N p = 0x%08x, size = %6d,  %s, Line: %4d.  totalsize =%8d\n", p, size, CString(file), line, _totalsize);
...
void DebugNewTracer::Remove (void* p)
{
    ...
    if (it != _map.end ())
    {
        ...
        //LYDBG(L"#D p = 0x%08x, size = %6u.%39stotalsize =%8d\n", p, size, "-----------------------------------  ", _totalsize );
        fflush(_logfp);
#endif
        _map.erase (it);
    }
