...
{
public:
	CXMLMarkup() { x_InitMarkup(); SetDoc( NULL ); };
	CXMLMarkup( MCD_CSTR szDoc ) { x_InitMarkup(); SetDoc( szDoc ); };
	CXMLMarkup( int nFlags ) { x_InitMarkup(); SetDoc( NULL ); m_nDocFlags = nFlags; };
	CXMLMarkup( const CXMLMarkup& markup ) { x_InitMarkup(); *this = markup; };
	void operator=( const CXMLMarkup& markup );
	~CXMLMarkup();
